...
// Returns a bearer token. With a valid saved session this is silent (delegated
// to the engine's createTokenProvider). Otherwise, unless `noInteractive` is
// set, it runs the interactive browser sign-in via the desktop's PKCE flow.
async function getAuthenticatedToken({ noInteractive = false, openBrowser } = {}) {
  ...
  const getToken = createTokenProvider({
    ...
    acquireInteractive: async () => {
      ...
      return runAuthFlow(openBrowser); // browser-PKCE, needs the openBrowser callback
    },
  });
  return getToken();
}
